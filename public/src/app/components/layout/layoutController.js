    ...
    function layoutController($scope, $rootScope, $commons, $logger, $cookies, fantumnService, exceptionService, $q, $interval, md5, $filter,$window,location) {
        ...
        $scope.initFunction = function() {
            try {
                ...
                $scope.guideenable =false;
                $scope.source_image='assets/images/step1.png';
                $scope.fin='NEXT';
                $scope.imgcount=1;
                $scope.content="Pitch a Match for Upcoming Tab";
                $scope.user = {
                    "username": "",
                    "password": ""
                };
       ...
       $scope.disable =function(){            
                                ...
                                for (var i = 0; i < $scope.upcome.data.length; i++) {
                                    if (currentDate < $scope.upcome.data[i].match.startingDateTime) {
                                        if($scope.upcome.data[i].season.competitionId==8){
                                            ...
                                            var t=$scope.upcome.data[i].match.startingDateTime;
                                             var a = t.split('T');                                              
        ...
        $scope.register = function(formValid) {
            ...
            if (formValid) {
                var va=$scope.userDetail.password;                
                if(va.length>=8){    
                if ($scope.agreeRegister) {                    
                    ...
                    $scope.toastContent = 'Password Must be minimum 8 Characters';
                    Materialize.toast($scope.toastContent, 3000);
                }
            }
        };
